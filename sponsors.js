...
const MARCAS = [
    // ── TIER 1: Fama < 30 ────────────────────────────────────────────
    {
        ...
        duracion: 2,
        bonus: { viralidad: 1 },
        descripcion: "Suplementos locales. Bono económico y +1 Viralidad pasiva.",
    ...
    },
    {
        ...
        duracion: 2,
        bonus: { viralidad: 1 },
        descripcion: "Tienda local de calzado deportivo. +1 Viralidad.",
...
function getOfertasDisponibles() {
    ...
    let disponibles = MARCAS.filter(m => {
        let yaFirmada = p.sponsorContratos && p.sponsorContratos.some(c => c.id === m.id && c.temporadasLeft > 0);
        let yaRechazada = p.sponsorRechazadas && p.sponsorRechazadas.includes(m.id + '_' + p.season);
        
        return !yaFirmada && !yaRechazada && p.fame >= m.minFame && p.fame <= m.maxFame && virActual >= m.minViralidad;
    });
...
function renderAgencia() {
    ...
    if (activos.length > 0) {
        ...
        activos.forEach(c => {
            ...
            html += `
            <div style="background:rgba(0,255,255,0.05);border:1px solid #333;border-radius:4px;padding:7px;margin-bottom:5px;font-size:0.58em;">
    ...
    html += `<div style="font-size:0.55em;color:gold;margin:8px 0 4px;letter-spacing:1px;">📨 OFERTAS DE ESTA TEMPORADA</div>`;
    ...
    if (ofertas.length === 0) {
        ...
        ofertas.forEach(m => {
            let yaFirmada = activos.some(c => c.id === m.id);
            ...
            html += `
            <div style="background:#0a0a0a;border:1px solid ${tierColor};border-radius:5px;padding:10px;margin-bottom:8px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                    <span style="font-size:0.75em;color:${tierColor};font-weight:bold;">${m.logo} ${m.nombre}</span>
                ...
                ${yaFirmada
                    ? `<div style="font-size:0.6em;color:#555;text-align:center;">Ya tienes contrato con esta marca.</div>`
...
function firmarPatrocinio(marcaId) {
    ...
    // Aplicar los bonos (SÓLO suma viralidad y fama, nada deportivo)
    ...
    if (m.fameBonus) {
        // En caso de que FAME_MAX no esté definido en game.js, aseguramos el límite de 100
...
function procesarContratosFinTemporada() {
    ...
    p.sponsorContratos.forEach(c => {
        if (c.temporadasLeft > 0) {
            c.temporadasLeft--;
            if (c.temporadasLeft === 0) {
                escribirDialogo(`📝 Tu contrato con <b>${c.nombre}</b> ha finalizado.`);
            }
        }
    });
    p.sponsorContratos = p.sponsorContratos.filter(c => c.temporadasLeft > 0);
